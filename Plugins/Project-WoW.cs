//THIS PLUGIN IS BASED ON THE AZENIUSHELPER PLUGIN! PLEASE THANKS THEM ;)

using System.Drawing;

namespace ProjectWoW
{
    using System;
    ...
    public class ProjectWoW : HBPlugin
    {

        #region Boring Plugin Info

        public override string Name { get { return "Project-WoW"; } }
        public override string Author { get { return "Copyright: Deadly, Shak & Megser"; } }
        ...
        {
            Logging.Write(System.Drawing.Color.Teal, "[ProjectWoW]: " + format, args);
        }
...
public List<WoWUnit> NorthridgeSpider
        {
            get
            {
                return ObjectManager.GetObjectsOfType<WoWUnit>().Where(u => u.Entry == 44284 && !Me.Dead).OrderBy(ret => ret.Distance).ToList();
            }
        }
public List<WoWUnit> Impsy
        {
            get
            {
                return ObjectManager.GetObjectsOfType<WoWUnit>().Where(ret => (ret.Entry == 47339 && !Me.Dead)).OrderBy(ret => ret.Distance).ToList();
            }
        }
...
public List<WoWUnit> Puddlejumper
        {
            get
            {
                return ObjectManager.GetObjectsOfType<WoWUnit>()
                                    .Where(u => ((u.Entry == 1543 || u.Entry == 1544) && u.CurrentTarget.HealthPercent < 25))
                                    .OrderBy(u => u.Distance).ToList();
            }
        }
        #endregion
...
*/
            ...
            if (IsOnQuest(13961) && IsOnQuest(13973))
            {
...
#region Quest 27001 -This means WAR *

            // @TODO - Find out why this doesn't check for quest completion

            if (Me.QuestLog.GetQuestById(27001) != null && !Me.Dead)
{
WoWPoint ToMill = new WoWPoint(2427.505, -1648.061, 104.1119);
WoWPoint ToSpiderHangout = new WoWPoint(2385.597, -1641.808, 106.445);
                {
Lua.DoString("PetPassiveMode()");
Navigator.MoveTo(ToSpiderHangout);
Thread.Sleep(10000);
                    NorthridgeSpider[0].Target();
                    Thread.Sleep(500);
                    Navigator.MoveTo(Me.CurrentTarget.Location);
Thread.Sleep(10000);
                    Lua.DoString("AttackTarget()");
                    Navigator.MoveTo(ToMill);
Thread.Sleep(5000);
                    Thread.Sleep(1000);
                }
            }
...
#region Quest 28000 - Do the Imp-Possible

if (Me.QuestLog.GetQuestById(28000) != null && !Me.Dead)
{
while (IsOnQuest(28000) && Me.CurrentTarget.HealthPercent < 2)
                {
Navigator.MoveTo(Me.CurrentTarget.Location);
Thread.Sleep(2000);
Impsy[0].Target();
...
#region Quest 25701 - You are Rakh'likh, Demon
...
if (Me.QuestLog.GetQuestById(24974) != null && !Me.Dead)
{
while (IsOnQuest(24974) && Me.CurrentTarget.HealthPercent < 2 && Puddlejumper.Count > 0)
                {
Lua.DoString("SpellStopCasting()"); // Stop casting all spells before using item
UseQuestItem(52059);
}
}
